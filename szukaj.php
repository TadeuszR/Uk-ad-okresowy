...
<?php
  ...
  if (!$uchwyt_polaczenia) {
	...
	$t = str_replace('ż','Z',$t);
	$t = str_replace('ź','Z',$t);
	$pierwiastek = pg_query($uchwyt_polaczenia, "select * from pierwiastki where upper(symbol) like '".$t."%' or upper(nazwapl) like '".$t."%'or upper(nazwalat) like '".$t."%' order by liczbaat;");

	if ($p = pg_fetch_assoc($pierwiastek)) 
	{
		echo	"<h2>Wyniki wyszukiwania</h2><br/>";
		echo	"<h3><a href=\"posymbolu.php?symbol=".$p[symbol]."\">".$p[symbol]." ".$p[nazwapl]."</a></h3>";		
	}
	else
